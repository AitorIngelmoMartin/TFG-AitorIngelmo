...
file_to_process = { 
    "files_to_process": [
        {
            "file_type":"Ex",
            "file_name_regular_expression":"(^(.*)_Ex\\.txt$)"
        },
        {
            "file_type":"Ey",
            "file_name_regular_expression":"(^(.*)_Ey\\.txt$)"
        },
        {
            "file_type":"Ez",
            "file_name_regular_expression":"(^(.*)_Ez\\.txt$)"
        }       
    ],
    "raw_data": 9,
    "freq":1.575e9,
...
def input_data_process(flow_config: dict):
    ...
    try:
        ...
        print(f'Se ha producido un error al extraer datos del fichero de configuración: {exc}')
...
def read_data(fields):
    ...
    for file_type, file_path in fields['files'].items():
        ...
        print(f'Lectura del fichero {file_path} perteneciente a la componente {file_type} realizada con éxito')
        ...
        print(f'Carga de los datos del fichero correcta')
...
def change_coordinate_system_to_spherical(fields: dict):
    ...
    print(f'Realizando interpolación de la componente Ex del campo')
    Ex_interpolated = Ex_interpolator(points_to_interpolate)

    print(f'Realizando interpolación de la componente Ey del campo')
    Ey_interpolated = Ey_interpolator(points_to_interpolate)

    print(f'Realizando interpolación de la componente Ez del campo')
...
def generate_spherical_grid_values(theta_init: int, theta_end: int, phi_init: int, phi_end: int, number_of_values: int, r: int = 1):
    ...
    print(f'Generando malla de valores en coordenadas esféricas')
...
def make_interpolator(fields: dict, field_component: str):
    ...
    print(f'Creación del interpolador de la componente del campo {field_component} completada')
...
def generate_points_to_interpolate(x_spherical: list, y_spherical: list, z_spherical: list):
    ...
    print(f'Generando puntos para hacer la interpolación')
...
def change_versor_coordinates_to_spherical(Ex_interpolated: list, Ey_interpolated: list, Ez_interpolated: list , r_grid: list, theta_grid: list, phi_grid: list):
    ...
    print(f'Modificando sistema de coordenadas de los resultados de hacer la interpolación a esféricas')
...
def translate_spherical_values_to_cartesians(r_grid: list, theta_grid: list, phi_grid: list):
    ...
    print(f'Modificando sistema de coordendadas de esférico a cartesiano')
...
def calculate_emn(E_r: list, E_theta: list, E_phi: list , theta_linspace: list, phi_linspace: list, number_of_modes: int, delta_theta: int, delta_phi: int):
    ...
    total_result = []
    threshold = 1e-10
    print(f'Iniciando cálculo de emn')
    ...
    print(f'Cálculo de emn completado')
    return total_result
...
def calculate_gmn(E_r: list, E_theta: list, E_phi: list , theta_linspace: list, phi_linspace: list, number_of_modes: int, delta_theta: int, delta_phi: int):
    ...
    total_result = []
    threshold = 1e-10
    print(f'Iniciando cálculo de gmn')
    for n in range(1, number_of_modes + 1):
        value_calculated = []
        for m in range(-n, n + 1):
            ...
            value_calculated.append(aux)
        total_result.append(value_calculated)
    print(f'Cálculo de gmn completado')
    return total_result
...
def calculate_bmnffcoef_from_emn(number_of_modes: int, emn: list, r: int, k: int):
    ...
    print(f'Iniciando cálculo de bmn')
    for n in range(1, number_of_modes + 1):
        value_calculated = []
        for m in range(-n, n + 1):
        ...
        total_result.append(value_calculated)
    print(f'Cálculo de bmn completado')
    return total_result
...
def calculate_amnffcoef_from_gmn(number_of_modes: int, gmn: list, r: int, k: int):
    ...
    total_result = []
    print(f'Iniciando cálculo de amn')
    for n in range(1, number_of_modes + 1):
        value_calculated = []
        for m in range(-n, n + 1):
            ...
            value_calculated.append(b_coef_value)
        total_result.append(value_calculated)
    print(f'Cálculo de amn completado')
    return total_result
...
def calculate_far_field(number_of_modes: int, a_coef_value: list, b_coef_value: list, theta_values, phi_values):
    ...
    print(f'Iniciando obtención del campo lejano')
    ...
    for theta in theta_values:
        ...
        for phi in phi_values:
            ...
            phi_index += 1
        theta_index += 1
    print(f'Obtención del campo lejano finalizada con éxito')
