...
def transformation_to_farfield(fields,measure_cut):
    ...
    kx_array = np.roll(np.arange(-Nx/2,Nx/2)*delta_kx,int(Nx))
    ky_array = np.roll(np.arange(-Ny/2,Ny/2)*delta_ky,int(Ny))
    #kx_array = np.arange(-Nx/2,Nx/2)*delta_kx
    #ky_array = np.arange(-Ny/2,Ny/2)*delta_kx 

    # Generación de los valores de los ángulos
    theta = np.linspace(0, np.pi, dimension)
    ...
    for i in range(len(fields['zValueZeroedplane'].keys())):
        ...
        if field_component in ['Ex','Ey','Ez','normE']:
            ...
            Ehat_component_interp_data = Ehat_component_interp_func(kxy)

            Ehat_component_calculated = f_factor*(Ehat_component_interp_data)
    ...
    extract_values_to_file(theta,phi,normE=fields['fields_transformed']['FF_normE'])
    #represent_radiation_map(phi_mesh,theta_mesh,suma)
def extract_values_to_file(theta,phi,normE):
...
"""
    with open('python_FF_normR.txt', 'w') as f:
        ...
        for i in range(len(theta)):
            for colum in range(normE.shape[0]):
                line = ("{:.8f}        {:.8f}       {:.8f}\n").format(theta[i],phi[colum],np.abs(normE[i,colum]))
                #line = ("{:.8f}        {:.8f}       {:.8f}\n").format(theta[colum,i],phi[colum,i],np.abs(normE[i,colum]))
                f.write(line)
                #f.write(f"{theta[i]}        {phi[i]}       {np.abs(normE[i,colum])}\n")
